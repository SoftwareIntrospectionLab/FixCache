...
public class CacheItem {
    ...
    "(select id from scmlog where repository_id=? and is_bug_fix=1 and date <=? and date >=?)";
    ...
    //called every time entityId moved into the cache
    ...
    private int findNumber(int entityId, int pid, String cdate, String sdate) {
        ...
        case BUGS:
            ret = findNumberOfBugs(entityId, pid, cdate, sdate);
            break;
        case CHANGES:
            ret = findNumberOfChanges(entityId, pid, cdate, sdate);
            break;
        case AUTHORS:
            ret = findNumberOfAuthors(entityId, pid, cdate, sdate);
            break;
        case LRU: // do nothing
        }
        return ret;
